...
//    Note *nota1 = [Note noteWithContext:self.managedObjectContext
//                                 inBook:<#(Notebook *)#>
//    NSLog(@"Una nota: %@", nota1);
//    
...
//    
//    request.sortDescriptors = @[[[NSSortDescriptor alloc] initWithKey:@"title" ascending:YES], [[NSSortDescriptor alloc] initWithKey:@"creationDate" ascending:NO]];
//    
//    NSError *err = nil;
//    
//    NSArray *notes =[self.managedObjectContext executeFetchRequest:request
//                                                             error:&err];
//    if (notes == nil) {
//        // Error al canto
//        NSLog(@"Error al buscar: %@", err);
//    }else{
//        NSLog(@"Resultado: %@", notes);
//    }
//    
//    // Eliminar
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    // Creo el controlador de tabla
    ATNotebookViewController *bookVC = [[ATNotebookViewController alloc] initWithContext:self.managedObjectContext];
    ...
    // Arranco el autoguardar
    [self performSelector:@selector(saveContext)
               withObject:nil
               afterDelay:AUTO_DELAY_SECONDS];
...
- (void)saveContext
{
    ...
    if (managedObjectContext != nil) {
        if ([managedObjectContext hasChanges] && ![managedObjectContext save:&error]) {
            ...
            if (AUTO_SAVE) {
                // Lo vuelvo a enviar en n segundos
                [self performSelector:@selector(saveContext)
                           withObject:nil
                           afterDelay:AUTO_DELAY_SECONDS];
            }
